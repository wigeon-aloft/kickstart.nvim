...
return {
	...
	config = function()
		...
		local godot_port = tonumber(os.getenv("GDScript_Port")) or 6005
		local godot_cmd = vim.lsp.rpc.connect("127.0.0.1", godot_port)
		local godot_pipe = "/path/to/godot.pipe" -- I use /tmp/godot.pipe

		local servers = {
			bashls = {},
			gopls = {
				buildFlags = { "-tags=local-dev-only" },
			},
			pyright = {},
			rust_analyzer = {},
			...
			-- 		vim.api.nvim_command('echo serverstart("' .. godot_pipe .. '")')
			-- 	end,
			-- },
		}
